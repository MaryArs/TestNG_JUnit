...
public class AppTest extends AppFixture {
    ...
    @Test (groups = "positive", dataProvider = "nameFilesWithoutExtension")
     //***Positive test for Linux Ubuntu. Create file with inexistent extension.
    public void CreateFileWithoutExtension(String nameFilesWithoutExtension) throws IOException {
        File file = new File(String.valueOf(path),String.valueOf(nameFilesWithoutExtension));
        Assert.assertFalse(file.exists());
        Assert.assertEquals(true, file.createNewFile());
        System.out.println("File is created!");
        file.delete();
    ...
    @DataProvider
    public Iterator<Object[]> nameFilesWithoutExtension(){
        List<Object[]> data = new ArrayList<>();
        for (int i = 0; i<3; i++){
            data.add(new  Object[]{
                    generateRandomWithoutExtensionFilesName()
            });
        }
        return data.iterator();
    }

    private Object generateRandomWithoutExtensionFilesName(){
        int leftLimit = 97; // letter 'a'
        int rightLimit = 122; // letter 'z'
        int targetStringLength = 10;
        Random random = new Random();
